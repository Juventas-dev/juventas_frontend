...
export default function SignIn({navigation}: SignUpScreenProps){
    ...
    const canGoNext = Name && ID && Pass && PassCheck && PhoneNum && CheckNum 
        && (Pass == PassCheck); // 인증번호 확인 단계도 거쳐야 함
    const onSubmit = useCallback(() => {
        if (canGoNext)
        Alert.alert('알림', '환영합니다.')
        else Alert.alert('알림', '부족')
    }, []);
    return (
        <DismissKeyboardView>
...
const styles = StyleSheet.create({
    ...
    title: {height: 60},
    titleHeader: {fontSize: 23, color: 'white', fontFamily: 'NotoSansKR-Bold', top: -20},
    titleBody: {color: '#C4C4C4', fontSize: 10, fontFamily: 'NotoSansKR-Regular', top: -30},
